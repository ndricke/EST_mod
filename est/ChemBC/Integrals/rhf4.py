...
for i in range(100):
 ...
 if len(err_DIIS) >= 2:
  lenE = len(err_DIIS)
  errMM = np.matrix(np.ones((lenE+1,lenE+1))*-1)
  errMM[lenE,lenE] = 0
  for j in np.arange(lenE):
   for k in np.arange(j+1):
    errMM[j,k] = np.sum(err_DIIS[k]*err_DIIS[j])
    errMM[k,j] = errMM[j,k]
  b = np.matrix(np.zeros((lenE+1,1)))
  b[-1] = -1
  w_vec = np.linalg.solve(errMM,b)
  Fp += [F_mat[j]*float(w_vec[j]) for j in range(lenE)]
 else:
  Fp = F

 Fprime = S_irT*Fp*S_ir
 ...
 if conv == True: break
 if i >= 2:
  err_DIIS.append(np.array(F*P_mat*S_orb_overlap - S_orb_overlap*P_mat*F))
  F_mat.append(F)
